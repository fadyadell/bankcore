...
export default function AdminTransactionsPage() {
  ...
  const fetchTasks = () => {
    ...
    apiClient.get('/tasks')
      .then(res => {
        ...
        const adminTxTasks = taskArray.filter((t: Task) => t.taskDefinitionKey?.toLowerCase().includes('admin') && t.variables?.transactionId);
        setTasks(adminTxTasks);
      })
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="mb-8">
        <h1 className="text-2xl font-bold tracking-tight text-slate-900">Final Transaction Review</h1>
        <p className="text-slate-500">Provide executive sign-off on escalated transactions.</p>
      </div>

      <Card>
        <CardHeader className="border-b border-slate-100 bg-slate-50/50 pb-4">
          <CardTitle>Transactions Awaiting Final Sign-off</CardTitle>
        </CardHeader>
              ...
              {loading ? (
                ...
                <TableRow>
                  <TableCell colSpan={5} className="py-12 text-center text-slate-500">
                    <span className="material-symbols-outlined mx-auto mb-2 text-3xl text-slate-400">task_alt</span>
                    <p>No transactions awaiting final review.</p>
                  </TableCell>
                </TableRow>
              ) : (
                tasks.map((task) => (

...
export default function AdminLoansPage() {
  ...
  const fetchTasks = () => {
    ...
    apiClient.get('/tasks')
      .then(res => {
        ...
        const adminLoanTasks = taskArray.filter((t: Task) => t.taskDefinitionKey?.toLowerCase().includes('admin') && t.variables?.loanId);
        setTasks(adminLoanTasks);
      })
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="mb-8">
        <h1 className="text-2xl font-bold tracking-tight text-slate-900">Final Loan Sign-off</h1>
        <p className="text-slate-500">Executive review for pre-approved loan applications.</p>
      </div>

      <Card>
        <CardHeader className="border-b border-slate-100 bg-slate-50/50 pb-4">
          <CardTitle>Applications Awaiting Final Sign-off</CardTitle>
        </CardHeader>
              ...
              {loading ? (
                ...
                <TableRow>
                  <TableCell colSpan={5} className="py-12 text-center text-slate-500">
                    ...
                    <p>No loan applications awaiting final review.</p>
                  </TableCell>
                </TableRow>
              ) : (
                tasks.map((task) => (
